...
dl_video() {
	local FMT="$1"
	local URL="$2"
	local DL_FLAGS="--no-playlist --add-metadata --geo-bypass -x --audio-format $FMT --audio-quality 0"
...
org_tags() {
	...
	for file in *; do
		...
		for i in `seq 0 4`; do
			if [[ $i = 0 ]]; then
				[[ -n "${VAL[$i]}" && "$ALBUM" != "album" ]] && ffmpeg \
					-hide_banner \
					-y \
					-i "$file" \
					-map 0 \
					-c copy \
					-metadata "${KEY[$i]}=${VAL[$i]}" "temp.$FMT" \
					2>/dev/null && cp -r "temp.$FMT" "$file" && rm -rf "temp.$FMT";
			else
				[[ -n "${VAL[$i]}" ]] && ffmpeg \
					-hide_banner \
					-y \
...
split() {
	...
	while read -r line; do
		...
		ffmpeg -hide_banner \
			...
			-i "$orig" \
			-c copy \
			-ss "$start_at" -to "$stop_at" "$track $title.$FMT" \
			2>/dev/null && track=`expr $track + 1`;\
	done < "$TIMESTAMPS" ;
	rm -f "$orig";
	cd .. ;
}
...
if [[ -n $SPLIT && -z $TIMESTAMPS ]]; then
	...
	[[ ! -s timestamps ]] && echo "error: timestamps empty" >&2 && exit 1;
...
case $TYPE in
	playlist) ALBUM="$(dl_playlist $FMT $URL)" && org_tags ;;
	watch)
		if [[ -n $SPLIT ]]; then
			dl_video $FMT $URL && split && org_tags;
		else
			dl_video $FMT $URL && org_tags;
		fi
	;;
esac
